...
This repo is a fork of the Alt Text dataset from Chintalapati et al. (ASSETS 2022). This repo focuses on analysis and export of the data.

## Setup

1. Run `uv sync` to install packages and create a virtual environment. Activate environment with `source .venv/bin/activate`.
1. If contibuting, run `pre-commit install` to setup code formatting and linting hooks.
...
1. Install Miniconda following the instructions [here](https://docs.conda.io/en/latest/miniconda.html)
1. From the repo home, run `conda env create -n hci-alt-texts -f environment.yml`
1. Run `conda activate hci-alt-texts`
1. Now you should be able to execute all of the analysis scripts. You may be prompted in some cases to install additional libraries or datasets.
