...
def train(data: List[Dict], outdir: str, option: str = "tfidf", num_folds: int = 5):
    ...
    n_scores_a = cross_val_score(
        multi_target_forest, X_vectors, y, scoring="accuracy", cv=cv, n_jobs=-1
    )
    n_scores_f1 = cross_val_score(
        multi_target_forest, X_vectors, y, scoring="f1_weighted", cv=cv, n_jobs=-1
    )
    ...
    print(
        "Mean F1 (weighted): %.3f (%.3f)" % (np.mean(n_scores_f1), np.std(n_scores_f1))
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--folds", type=int, help="Number of folds for cross-validation", default=5
    )
    parser.add_argument(
        "--option", type=str, help="Vectorization options (tfidf, spacy)"
    )
